...
export default function HeroAssembly() {
  ...
  useEffect(() => {
    ...
    function drawDoc(
      ...
      c.stroke();
      c.font = "600 8.5px " + MONO;
      c.textBaseline = "alphabetic";
      c.textAlign = "left";
      c.fillStyle = PAL.dim;
      c.fillText(d.name, 9, 15);
      const tagW = c.measureText(d.tag).width + 10;
      const cc = chipCols();
      rr(c, w - 9 - tagW, 6, tagW, 12, 3);
      c.fillStyle = cc.bg;
      c.fill();
      c.fillStyle = cc.fg;
      c.font = "600 7.5px " + MONO;
      c.textAlign = "center";
      c.fillText(d.tag, w - 9 - tagW / 2, 15);
      c.textAlign = "left";
      c.strokeStyle = PAL.line;
      c.lineWidth = 0.8;
      c.beginPath();
    ...
    function drawSystem(c: CanvasRenderingContext2D, live: boolean) {
      ...
      for (let i = 0; i < ROWS.length; i++) {
        ...
        const barX = g.pX + padX + 66;
        const barW = g.pW - padX * 2 - 66 - 56;
        if (barW > 20) {
          c.strokeStyle = PAL.line;
          c.lineWidth = 6;
          c.lineCap = "round";
