...
/* s-1022 probe. draw() reads the DOM the stage just wrote: the picture is
   chosen by the caption text on the page, not by cycT, so it lags the words
   by a frame and follows anything else that writes the caption. */
const CYCLE = 22;
const STANDING = 14;
...
function cellarScene(captions: Record<"a" | "b", string>): DemoScene {
  return {
    ...
    layout: () => {},
    draw(env, cycT) {
      const said = document.querySelector(".demo-caption")?.textContent ?? "";
      const t = said === captions.b ? STANDING : Math.min(cycT, STANDING);
...
export default function DomReadDemo() {
