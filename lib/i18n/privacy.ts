...
const en = {
  ...
  eyebrow: "Legal",
  title: "Privacy Policy",
  effective: "Effective 2026-09-24. Applies to cardondigital.com.",
  intro:
  ...
  contactBody:
    "There is always a written way to reach us, and it has two shapes: the contact form where this site can deliver mail itself, and a plain link that opens your own email client where it cannot. Only one of them is ever on the page. WhatsApp and the booking link, when the page shows them, hand you over to those services under their own policies. Through the form you give us your name, your email address and your message, plus your winery or business and your WhatsApp number if you choose to fill them in, and it reaches our own inbox through the service that delivers mail for this site, which handles it only in order to deliver it. The form sends two things you did not type: the page you were on when you wrote, and whatever marks that page's own address was carrying, which is how we learn which of our own pages and links start conversations. Those marks are read from the address of the page you write us from and from nowhere else, so if you arrived on a marked link and then read a while before writing, there is no mark left to send. For each message we also write one line to our server log, holding the time, your language, the winery or business name if you gave one and two of those marks, the click identifier and the lane, and never your name, your address or what you wrote; if a message cannot be delivered, that line also carries the first letter of your name, the domain of your address and how long your message was in characters, so a failure is visible without keeping a second copy of your message. If you write by email instead, we receive your address and whatever you choose to tell us; the subject line we prepare carries that same lane in brackets, which you can read and delete before you send, and the WhatsApp and booking links carry it the same way. Either way we use it to reply and, if we end up working together, to deliver the work. We keep correspondence as ordinary business records, we do not add you to mailing lists you did not ask for, and we never sell or share your details for marketing.",
  ...
  clientH: "Client data",
  clientBody:
    "In client engagements we routinely work inside business systems that contain personal data (for example customer or applicant records). That data is processed under the client's instructions and our written agreement with them: access is limited to the people doing the work, it is used only to deliver the engagement, and it is never extracted for any other purpose. Where an engagement runs a system for a client, our written agreement keeps that system's data, and every backup of it, in accounts in the client's own name: a backup each day where the system runs, kept for the period that provider's plan gives, and a copy each week with a second storage provider, also in the client's name, kept thirty days, so no single provider's failure can lose the record. We hold no rolling copy of a client's data ourselves, and none of it is data this website collects or touches: everything above about forms, logs and cookies is about this site alone. Questions about a specific company's data should go to that company; we support our clients in answering them.",
  rightsH: "Your rights",
  rightsBody:
...
const es: typeof en = {
  ...
  title: "Aviso de Privacidad",
  effective:
    "Vigente desde el 24 de septiembre de 2026. Aplica a cardondigital.com.",
  intro:
  ...
  clientBody:
    "En los trabajos con clientes entramos de forma habitual a sistemas del negocio que contienen datos personales (por ejemplo registros de clientes o de candidatos). Esos datos se tratan bajo las instrucciones del cliente y nuestro acuerdo escrito con él: el acceso se limita a quien hace el trabajo, se usa solo para entregar el proyecto y nunca se extrae para otro fin. Cuando un trabajo opera un sistema para un cliente, nuestro acuerdo escrito mantiene los datos de ese sistema, y cada respaldo de ellos, en cuentas a nombre del propio cliente: un respaldo diario donde corre el sistema, guardado por el periodo que dé el plan de ese proveedor, y una copia semanal con un segundo proveedor de almacenamiento, también a nombre del cliente, guardada treinta días, para que la falla de un solo proveedor no pueda perder el registro. Nosotros no guardamos ninguna copia continua de los datos de un cliente, y nada de eso son datos que este sitio recabe ni toque: todo lo anterior sobre formularios, registros y cookies es de este sitio solamente. Las preguntas sobre los datos de una empresa en particular van dirigidas a esa empresa; nosotros la apoyamos para responderlas.",
